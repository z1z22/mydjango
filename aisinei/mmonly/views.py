...
def tag(request):
    taglist = MmonlyTag.objects.all()
    tags = []
    for tag in taglist:
        titlesnum = tag.mmonlytitle_set.count()
        tag_dict = {
            'tag':tag.tag,
            'num':titlesnum,
            'id':tag.id
    ...
    return render(request, 'mmonly/tag.html', {'tags':tags})


def title(request, tagid):
    #去模板取数据
    tag = get_object_or_404(MmonlyTag, id=tagid)
    titlelist = tag.mmonlytitle_set.order_by('id')
    ttlist = []
    for title in titlelist:
        pic = title.mmonlypic_set.first()
        picsnum = title.mmonlypic_set.count()
    ...
    return render(request, 'mmonly/title1.html', {'titles': titles, 'tagid' :tagid, 'tag':tag})
...
def list_title(request, tagid):
    ...
    if request.method == "GET":
        ...
        # todo: 注意捕获异常
            ...
            # 如果请求的页数不在合法的页数范围内，返回结果的最后一页。
            titles = paginator.page(paginator.num_pages)
    return render(request, 'mmonly/title.html', {'titles': titles,  'tag':tag})

def pic(request, tagid, titleid,):
    tag = get_object_or_404(MmonlyTag, id=tagid)
    title = get_object_or_404(MmonlyTitle, id=titleid)
    piclist = title.mmonlypic_set.order_by('picname')
    # print(request.body)
    # print('*'*80)
    ...
    return render(request, 'mmonly/picture1.html', {'pics':pics,'tag':tag, 'title':title,})
...
def pic_big(request, tagid, titleid,):
    ...
    return render(request, 'mmonly/picture.html', {'pics':pics,'tag':tag, 'title':title, })
def search(request):
    q = request.GET.get('q')
    error_msg = ''
    if not q:
        error_msg = '请输入关键词'
        return render(request, 'mmonly/error.html',{'error_msg':error_msg})
    titles = MmonlyTitle.objects.filter(title__icontains=q)
    number = len(titles)
    if number > 30:
        titles = titles[:30]
    ...
    return render(request, 'mmonly/search_title.html',{'error_msg': error_msg, 'titles': titles})
