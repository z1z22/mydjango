...
def tag(request):
    taglist = MeituriTag.objects.all()
    tags = []
    for tag in taglist:
        titlesnum = tag.meiturititle_set.count()
        tag_dict = {
            'tag':tag.tag,
            'num':titlesnum,
            'id':tag.id
    ...
    return render(request, 'meituri/tag.html', {'tags':tags})


def title(request, tagid):
    #去模板取数据
    tag = get_object_or_404(MeituriTag, pk=tagid)
    titlelist = tag.meiturititle_set.order_by('id')
    ttlist = []
    for title in titlelist:
        pic = title.meituripic_set.first()
        picsnum = title.meituripic_set.count()
    ...
    return render(request, 'meituri/title1.html', {'titles': titles, 'tagid' :tagid, 'tag':tag})
...
def list_title(request, tagid):
    ...
    if request.method == "GET":
        ...
        # todo: 注意捕获异常
            ...
            # 如果请求的页数不在合法的页数范围内，返回结果的最后一页。
            titles = paginator.page(paginator.num_pages)
    return render(request, 'meituri/title.html', {'titles': titles,  'tag':tag})

def pic(request, tagid, titleid,):
    tag = get_object_or_404(MeituriTag, pk=tagid)
    title = get_object_or_404(MeituriTitle, pk=titleid)
    piclist = title.meituripic_set.order_by('picname')
    # print(request.body)
    # print('*'*80)
    ...
    return render(request, 'meituri/picture1.html', {'pics':pics,'tag':tag, 'title':title,})
...
def pic_big(request, tagid, titleid,):
    ...
    return render(request, 'meituri/picture.html', {'pics':pics,'tag':tag, 'title':title, })
def search(request):
    q = request.GET.get('q')
    error_msg = ''
    if not q:
    ...
    titles = MeituriTitle.objects.filter(title__icontains=q)
    number = len(titles)
    if number > 30:
        titles = titles[:30]
    ...
    return render(request, 'meituri/search_title.html',{'error_msg': error_msg, 'titles': titles})
